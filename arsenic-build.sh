...
KERNEL="zImage"
DTBIMAGE="dtb"
DEFCONFIG="arsenic_defconfig"
device="bacon"

# Kernel Details
BASE_ARSENIC_VER="Arsenic"
VER="Arsenic-test"
ARSENIC_VER="$BASE_Arsenic_VER$VER"

# Vars
export LOCALVERSION="-$ARSENIC_VER-$(date +%Y%m%d)"
export CROSS_COMPILE="/home/jc/cm13/prebuilts/gcc/linux-x86/arm/arm-eabi-4.9/bin/arm-eabi-"
export ARCH=arm
export SUBARCH=arm
...
FINAL_ZIP="/home/jc/oneplus/$ARSENIC_VER-$(date +%Y%m%d).zip"
# Functions
...
function make_zip {
		cd $REPACK_DIR
		zip -r9 ~/oneplus/$ARSENIC_VER-$(date +%Y%m%d).zip *
		cd $KERNEL_DIR
